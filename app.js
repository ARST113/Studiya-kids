...
const observer=new IntersectionObserver(entries=>entries.forEach(e=>{if(e.isIntersecting){e.target.classList.add('show');observer.unobserve(e.target)}}),{threshold:.08});document.querySelectorAll('.fade').forEach(el=>observer.observe(el));
document.getElementById('signupForm').addEventListener('submit',e=>{e.preventDefault();const f=new FormData(e.currentTarget);if(!f.get('consent'))return;const subject=encodeURIComponent('Заявка с сайта Студия Kids');const body=encodeURIComponent(`Имя родителя: ${f.get('parent')}\nТелефон: ${f.get('phone')}\nEmail: ${f.get('email')||'—'}\nИмя ребёнка: ${f.get('child')||'—'}\nВозраст: ${f.get('age')}\nНаправление: ${f.get('course')}\nФилиал: ${f.get('branch')}\nКомментарий: ${f.get('comment')||'—'}\n\nПользователь подтвердил согласие на обработку персональных данных через обязательный чекбокс формы.`);location.href=`mailto:studiokids@mail.ru?subject=${subject}&body=${body}`});